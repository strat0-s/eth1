...
contract KeyManagerV3 is Initializable, OwnableUpgradeable {
    ...
    function deletePublicKey(string memory userId) public onlyOwner {
        bytes32 userIdHash = keccak256(abi.encodePacked(userId));
        require(registeredUsers[userIdHash], "User ID is not registered");

        delete publicKeys[userIdHash];
        delete registeredUsers[userIdHash];
    }
